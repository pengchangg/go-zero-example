...
var (
	...
	cacheEntDemoUsersIdPrefix = "cache:entDemo:users:id:"
)
...
func (m *defaultUsersModel) Delete(ctx context.Context, id int64) error {
	entDemoUsersIdKey := fmt.Sprintf("%s%v", cacheEntDemoUsersIdPrefix, id)
...
func (m *defaultUsersModel) FindOne(ctx context.Context, id int64) (*Users, error) {
	entDemoUsersIdKey := fmt.Sprintf("%s%v", cacheEntDemoUsersIdPrefix, id)
	var resp Users
	err := m.QueryRowCtx(ctx, &resp, entDemoUsersIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	entDemoUsersIdKey := fmt.Sprintf("%s%v", cacheEntDemoUsersIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?)", m.table, usersRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Age, data.Name)
	}, entDemoUsersIdKey)
	return ret, err
}
...
func (m *defaultUsersModel) Update(ctx context.Context, data *Users) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Age, data.Name, data.Id)
	}, entDemoUsersIdKey)
	return err
}

func (m *defaultUsersModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheEntDemoUsersIdPrefix, primary)
}
